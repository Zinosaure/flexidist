...
*
*/
class HTML extends \TypeHint\Schema {
    ...
    */
    ...
    public static $TEMPLATES_PATH = TEMPLATES_PATH;

    /**
    *
    */
    public function __construct(?string $document = null, array $context_vars = []) {
        if (!is_null($document) && preg_match('/\.phtml$/isU', $document) && is_file(self::$TEMPLATES_PATH . $document))
            $document = file_get_contents(self::$TEMPLATES_PATH . $document);
        ...
        $patterns = [
            '/{%\s+(include|include_once|require|require_once)\s*\("(.+)"\)\s+%}/isU' => function(string $document, array $matches): string {
                foreach ($matches as $i => $match)
                    $document = str_replace($match[0], eval(sprintf('%s("%s");', $match[1], self::$TEMPLATES_PATH . $match[2])), $document);
            ...
            '/{%\s+extends\s*"(.+)"\s+%}/isU' => function(string $document, array $matches) use ($source_filename, $extends_limit): string {
                foreach ($matches as $i => $match) {
                    ...
                    else if (($content = @file_get_contents(self::$TEMPLATES_PATH . $match[1])) !== false)
