...
final class RouterHttp extends \Schema {
    ...
    */
    public function __construct(\RouterHttp\SecurityControl $SecurityControl = null) {
        ...
        $this->map('get', 'rsrc/*:filename', function(string $filename) {
            if (!is_file($filename = sprintf('%sviews/%s', APPLICATION_PATH, trim($filename))))
                $this->Response->status_code = 404;
            else {
                $extension = explode('.', $filename);
                $extension = strtolower(end($extension));
    
                $this->Response->headers->{'Content-Type'} = self::FILES_MIME_TYPES[$extension] ?? 'text/plain';
    ...
    */
    public function execute(\Closure $callback = null, array $callback_args = []) {
        ...
        foreach ((new \ReflectionFunction($callback))->getParameters() as $param)
            if (($param_type = $param->getType()) && !in_array($class_name = $param_type->getName(), ['int', 'string', 'array']))
    ...
    */
    public function listen(?string $http_request = null, ?string $method = null) {
        $this->dispatch($http_request, $method);
        $this->execute();
    }
